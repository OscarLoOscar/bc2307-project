...
// import com.hkjava.stock.trade.hub.model.Trader;
// import com.hkjava.stock.trade.hub.service.TradeService;
...
//     @Override
//     public List<OrderBookDTO> findOrders() {
...
//         if (BuySell.valueOf(buySell.toUpperCase()) == Action.BUY) {
...
//                     stockRepository.save(TradeRecord.builder().price(Trader.demoProduct.getAsk().getPrice()).quantity(quantity).localDateTime(LocalDateTime.now()).build());
//                     quantity = 0;
...
//                     stockRepository.save(TradeRecord.builder().price(Trader.demoProduct.getAsk().getPrice()).quantity(Trader.demoProduct.getAsk().getQuantity()).localDateTime(LocalDateTime.now()).build());
//                     // 隊remove一個盤，去下一個繼續成交
//                     Trader.demoProduct.removeAsk();
//                 }
//             }
//         }
//         if (BuySell.valueOf(buySell.toUpperCase()) == Action.SELL) {
//             if (trader.getProducts().get(Trader.demoProduct) == null || 
//             trader.getProducts().get(Trader.demoProduct) < quantity) {
...
//                         stockRepository.save(TradeRecord.builder().price(Trader.demoProduct.getBid().getPrice()).quantity(quantity).localDateTime(LocalDateTime.now()).build());
//                         quantity = 0;
//                     } else {
...
//                         stockRepository.save(TradeRecord.builder().price(Trader.demoProduct.getBid().getPrice()).quantity(Trader.demoProduct.getBid().getQuantity()).localDateTime(LocalDateTime.now()).build());
...
//         // Trader trader = new Trader(); // 有bean
//         if (BuySell.valueOf(buySell.toUpperCase()) == Action.BUY && trader.getMoney() < price * quantity)
//                 throw new FinnhubException(Code.TRADER_NOTENOUGH_MONEY);
//         if (BuySell.valueOf(buySell.toUpperCase()) == Action.SELL && (trader.getProducts().get(Trader.demoProduct) == null || trader.getProducts().get(Trader.demoProduct) < quantity))
//                 throw new FinnhubException(Code.TRADER_NOTENOUGH_PRODUCT);
